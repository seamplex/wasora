...
int wasora_init_before_parser(void) {
...
// mira si los n puntos en x[], y[] y z[] forman una grilla estruturada
/*
int wasora_is_structured_grid_xlast(int n_args, int n_points, double *arg, int *size) {
 
  int i, j;
  
  // checkeo rapido:
  if ((n_points % n_args) != 0) {
    return 0;
  }
  
  // primer paso, barremos para ver si dan los tamanios
  for (i = 0; i < n_args; i++) {
    
    // miramos cuando es la primera vez que cambia el argumento
    if (j >= n_points-1) {
      return 0;
    }
    size[i] = 1;
    j = 0;
    while (gsl_fcmp(arg[j], arg[j+1])
