...
The format for running the `wasora` program is:
...
may be solved by writing these three
...
As detailed in @sec:design, on the one hand wasora uses the UNIX idea of
...
divert(-1)
$$
\frac{B}{A}(A,Z) = a_1 - a_2 \cdot A^{-1/3} - a_3 \cdot Z(Z-1) A^{-4/3} - a_4 \cdot (A-2Z)^2 A^{-2} + a_5 \cdot \delta \cdot A^{-\gamma}
$$
...
as wasora was born and designed within the UNIX philosophy.
The usage of wasora in non-UNIX
...
@sec:history because it is the language I feel most
...
which wasora was first born and conceptually designed. 
...
dnl include(reference-manual.md)
