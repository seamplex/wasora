...
void wasora_set_function_args(function_t *function, double *x) {
  int i;
  
  if (function->var_argument != NULL) {
    for (i = 0; i < function->n_arguments; i++) {
      if (function->var_argument[i] != NULL) { 
        wasora_var(function->var_argument[i]) = x[i];
      }
    }
  }
...
double wasora_evaluate_factor_function(factor_t *token) {
...
//   wasora_push_error_message("when computing function %s", token->function->name);

  if (token->function->n_arguments == 1) {

    // para no tener que hacer malloc y frees todo el tiempo, para un solo argumento lo
    // hacemos diferente
    x0 = wasora_evaluate_expression(&token->arg[0]);
    y = wasora_evaluate_function(token->function, &x0);
    ...
    for (i = 0; i < token->function->n_arguments; i++) {
      x[i] = wasora_evaluate_expression(&token->arg[i]);
    }

    y = wasora_evaluate_function(token->function, x);
...
int wasora_function_init(function_t *function) {
  ...
  if (function->type == type_pointwise_vector) {
    for (i = 0; i < function->n_arguments; i++) {
      ...
      if (!function->vector_argument[i]->initialized) {
        wasora_call(wasora_vector_init(function->vector_argument[i]));
      }
      ...
      if (function->data_size == 0) {
        ...
        if (function->vector_argument[i]->size != function->data_size) {
          wasora_push_error_message("vector sizes do not match (%d and %d) in function '%s'", function->data_size, function->vector_argument[i]->size, function->name);
  ...
  if (function->vector_value != NULL) {
    ...
    if (!function->vector_value->initialized) {
      wasora_call(wasora_vector_init(function->vector_value));
    }
    if (function->vector_value->size != function->data_size) {
      wasora_push_error_message("vector sizes do not match (%d and %d)", function->data_size, function->vector_value->size);
    ...
    if (function->n_arguments == 1) {
      ...
      if ((dummy_var = wasora_get_variable_ptr(dummy_aux)) == NULL) {
        return WASORA_PARSER_ERROR;
      }
    ...
    function->data_value = gsl_vector_ptr(wasora_value_ptr(function->vector_value), 0);
...
double wasora_evaluate_function(function_t *function, const double *x) {
  ...
  if (function->type == type_pointwise_mesh_node) {
  ...
  } else if (function->type == type_plugin_routine) {
    y = function->plugin_routine(x, function->name);
    ...
    x_old = wasora_value(function->var_argument[0]);
    wasora_value(function->var_argument[0]) = x[0];
    y = wasora_evaluate_expression(&function->algebraic_expression);
    wasora_value(function->var_argument[0]) = x_old;
    ...
    for (i = 0; i < function->n_arguments; i++) {
      vecx_old[i] = wasora_value(function->var_argument[i]);
      wasora_value(function->var_argument[i]) = x[i];
    }

    y = wasora_evaluate_expression(&function->algebraic_expression);

    for (i = 0; i < function->n_arguments; i++) {
      wasora_value(function->var_argument[i]) = vecx_old[i];
    }
    ...
    if (function->multidim_interp == nearest) {
      ...
      if (flag == 0) {
        if (den == 0) {
          wasora_push_error_message("denominator equal to zero");
          wasora_runtime_error();
        }
        y = num/den;
      }
      ...
      if (flag == 0) {
        if (den == 0) {
          if (function->n_arguments == 3) {
            wasora_push_error_message("no definition point found in a range %g around point (%g,%g,%g), try a larger SHEPARD_RADIUS", function->shepard_radius, x[0], x[1], x[2]);
          } else {
            wasora_push_error_message("no definition point found in a range %g around point, try a larger SHEPARD_RADIUS", function->shepard_radius);
          }
        }
        y = num/den;
      }
      ...
      if (flag) {
        ...
        y = function->data_value[wasora_structured_scalar_index(function->n_arguments, function->rectangular_mesh_size, c, function->x_increases_first)];
      } else {
        ...
        for (i = 0; i < (1<<function->n_arguments); i++)  {
          ...
          index = wasora_structured_scalar_index(function->n_arguments, function->rectangular_mesh_size, ctilde, function->x_increases_first);
...
int wasora_structured_scalar_index(int n_dims, int *size, int *index, int x_increases_first) {
  int i, j;
  int k;
  int scalar_index = 0;
